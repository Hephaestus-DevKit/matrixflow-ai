...
也可以使用不读取本地变量的函数发布命令。它会读取受审查的 `ignore` 清单，排除
`node_modules`、Git 元数据、覆盖率和日志文件，同时保留 Console 中的 Secret：
...
日常发布禁止加 `--with-variables`：该 CLI 选项会用本地清单替换函数变量，可能清掉 Console 中的 Secret。仓库不会追踪 `vars` 清单，并由 `schema:check` 阻止重新加入。生产流水线使用幂等部署脚本，只更新受管的非敏感变量并保留模型密钥。当前线上还需要保留 `MATRIXFLOW_AGENT_LIMIT`、`MATRIXFLOW_CONTENT_PROJECT_LIMIT`、`MATRIXFLOW_KNOWLEDGE_BASE_LIMIT`、`MATRIXFLOW_WORKFLOW_LIMIT` 四个资源额度变量。
