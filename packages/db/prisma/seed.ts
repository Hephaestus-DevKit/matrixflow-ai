...
async function main() {
  ...
  const plans = [
    ...
    },
    {
      ...
      kbLimit: 10,
      kbDocLimit: 5000,
      features: { marketplace: 'both', brandVoice: true },
    },
    {
      ...
      kbLimit: 50,
      kbDocLimit: -1,
      features: { marketplace: 'both', payout: true },
    },
    {
      ...
      kbLimit: -1,
      kbDocLimit: -1,
      features: { marketplace: 'custom', sso: true, privateDeploy: true },
    },
  ];
  ...
  const prompts = [
    {
      ...
      category: 'ecommerce',
      systemPrompt:
        '你是跨境电商资深文案专家，擅长写出符合平台 SEO 规则且高转化的商品标题。严格遵守平台字数与禁用词规则。',
      userPromptTemplate:
    ...
    },
    {
      ...
      category: 'rag',
      systemPrompt:
        '你是知识库问答助手。只依据提供的上下文回答，若上下文无答案请明说"未在知识库中找到"。引用来源编号 [docN]。',
      userPromptTemplate:
  ...
  for (const p of prompts) {
    await prisma.promptTemplate.upsert({
      where: { key: p.key },
      update: p,
      create: { ...p, version: 1, isLatest: true },
    });
  }
  ...
  const agentTemplates = [
    {
      ...
      systemPrompt: { templateKey: 'product_title', variables: {} },
      defaultSkills: ['content:product_title', 'content:listing'],
      defaultTools: [],
      category: 'ecommerce',
      tags: ['amazon', 'shopify'],
    },
    {
      ...
      systemPrompt: { templateKey: 'tiktok_script', variables: {} },
      defaultSkills: ['content:tiktok_script'],
      defaultTools: [],
      category: 'social',
      tags: ['tiktok'],
    },
    {
      ...
      description: '基于知识库的 RAG 客服',
      systemPrompt: { templateKey: 'rag_qa', variables: {} },
      defaultSkills: ['rag:qa'],
      defaultTools: ['query_kb'],
      category: 'support',
      tags: ['support', 'rag'],
    },
  ];
