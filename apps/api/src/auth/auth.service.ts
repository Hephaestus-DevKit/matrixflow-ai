...
import { Action, ADMIN_ACTIONS, ErrorCode, MEMBER_ACTIONS, RoleName } from '@matrixflow/shared';
...
export class AuthService {
  ...
  async syncUser(
    appwriteUser: { $id: string; email: string; name?: string; emailVerification?: boolean },
    targetOrgId?: string,
  ) {
    ...
    const user = await this.prisma.$transaction(async (tx: any) => {
      ...
      const linked = await tx.account.findUnique({
        where: {
          provider_providerAccountId: { provider: 'appwrite', providerAccountId: appwriteId },
        },
        include: { user: true },
      });
      ...
      const resolvedUser =
        existing ?? (await tx.user.create({ data: { email, name, status: 'ACTIVE' } }));
      await tx.account.create({
        data: { provider: 'appwrite', providerAccountId: appwriteId, userId: resolvedUser.id },
      });
      return resolvedUser;
    });
    ...
    if (targetOrgId) {
      membership = await this.prisma.organizationMember.findFirst({
        where: {
          ...
          organization: { deletedAt: null, status: { not: 'SUSPENDED' } },
        },
  ...
  async register(input: unknown, ip?: string, ua?: string) {
    ...
    const result = await this.prisma.$transaction(async (tx: any) => {
      ...
      const membership = await this.createDefaultOrganization(
        tx,
        user.id,
        dto.organizationName ?? `${dto.name}'s Team`,
      );
      return {
        user,
  ...
  async refresh(refreshToken: string) {
    ...
    const refreshState = await this.redis.get<
      string | { sessionId: string; organizationId: string }
    >(`refresh:${tokenHash}`);
    ...
    const organizationId =
      typeof refreshState === 'string' ? undefined : refreshState.organizationId;
  ...
  async logout(refreshToken: string) {
    const tokenHash = this.hashToken(refreshToken);
    const refreshState = await this.redis.get<string | { sessionId: string }>(
      `refresh:${tokenHash}`,
    );
    const sid = typeof refreshState === 'string' ? refreshState : refreshState?.sessionId;
    if (sid) {
  ...
  async me(userId: string) {
    const user = await this.prisma.user.findUnique({
      ...
      include: {
        memberships: {
          where: { organization: { deletedAt: null, status: { not: 'SUSPENDED' } } },
  ...
  async updateProfile(userId: string, data: { name?: string; avatarUrl?: string }) {
    ...
    if (data.name !== undefined && (!name || name.length > 100))
      throw new BadRequestException('Name must contain 1-100 characters');
  ...
  private async createDefaultOrganization(tx: any, userId: string, organizationName: string) {
    ...
    const organization = await tx.organization.create({
      data: { slug, name: organizationName, plan: 'FREE' },
    });
    const owner = await tx.role.create({
      data: { organizationId: organization.id, name: RoleName.OWNER, isSystem: true },
    });
    const admin = await tx.role.create({
      data: { organizationId: organization.id, name: RoleName.ADMIN, isSystem: true },
    });
    const member = await tx.role.create({
      data: { organizationId: organization.id, name: RoleName.MEMBER, isSystem: true },
    });
    const ownerPermissions = Object.values(Action);
    const adminPermissions = ADMIN_ACTIONS;
    const memberPermissions = MEMBER_ACTIONS;
    await tx.permission.createMany({
      data: ownerPermissions.map((action) => ({ roleId: owner.id, action })),
    });
    await tx.permission.createMany({
      data: adminPermissions.map((action) => ({ roleId: admin.id, action })),
    });
    await tx.permission.createMany({
      data: memberPermissions.map((action) => ({ roleId: member.id, action })),
    });
