...
export class BillingService {
  ...
  async plans() { return this.prisma.plan.findMany({ where: { isActive: true }, orderBy: { priceMonthlyUsd: 'asc' } }); }

  async current(organizationId: string) {
    return this.prisma.subscription.findFirst({ where: { organizationId, status: 'active' }, include: { plan: true } });
  }
  ...
  async usage(organizationId: string, metric?: string) {
    const since = new Date(); since.setDate(1); // 本月
    const records = await this.prisma.usageRecord.findMany({ where: { organizationId, recordedAt: { gte: since }, ...(metric ? { metric } : {}) } });
    const agg: Record<string, number> = {};
    for (const r of records) agg[r.metric] = (agg[r.metric] ?? 0) + r.value;
    return agg;
  }

  async invoices(organizationId: string) { return this.prisma.invoice.findMany({ where: { organizationId }, orderBy: { createdAt: 'desc' } }); }

  async tokenUsage(organizationId: string) {
    const since = new Date(); since.setDate(1);
    return this.prisma.tokenUsage.findMany({ where: { organizationId, createdAt: { gte: since } }, orderBy: { createdAt: 'desc' }, take: 100 });
  }
}
