...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async set(key: string, val: unknown, ttlSec?: number) {
    const s = JSON.stringify(val);
    if (ttlSec) await this.client.set(this.k(key), s, 'EX', ttlSec);
    else await this.client.set(this.k(key), s);
  }
  async del(key: string) {
    await this.client.del(this.k(key));
  }
  async incr(key: string, ttlSec = 60): Promise<number> {
    const n = await this.client.incr(this.k(key));
    if (n === 1) await this.client.expire(this.k(key), ttlSec);
    return n;
  }
