...
const callInternal = createInternalClient({ baseUrl: internalApiUrl, secret: internalJobSecret, timeoutMs: requestTimeoutMs });
...
const documentWorker = new Worker<{ docId: string }>('document-processing', async (job) => {
...
}, { connection: createConnection() as unknown as ConnectionOptions, concurrency: Math.min(4, concurrency) });

const workflowWorker = new Worker<{ runId: string; userId: string }>('workflow-execution', async (job) => {
  logger.info({ jobId: job.id, runId: job.data.runId }, 'Executing workflow');
  return callInternal(`/internal/jobs/workflows/${encodeURIComponent(job.data.runId)}/execute`, { userId: job.data.userId });
