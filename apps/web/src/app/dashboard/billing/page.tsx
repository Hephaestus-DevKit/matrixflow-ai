    ...
    <div className="space-y-8">
      ...
      {current && (
        <div className="rounded-lg border border-success/30 bg-success/5 p-4 text-sm">
          当前套餐：<b>{current.plan?.name}</b> · {current.status}
        </div>
      )}
      <div>
        <h2 className="mb-3 text-sm font-semibold text-muted-foreground">本月用量</h2>
        ...
        {plans?.map((p: any) => (
          <div key={p.id} className="rounded-xl border border-border bg-card p-5">
            ...
            <ul className="mt-3 space-y-1 text-sm text-muted-foreground">
              <li>{p.seats} 席位</li>
              <li>{p.aiCallsPerMonth} AI 调用</li>
              <li>{p.workflowLimit} 工作流</li>
            </ul>
            <button
              className="mt-4 w-full rounded-md bg-primary py-2 text-sm text-primary-foreground"
              onClick={() => apiClient.post('/billing/subscribe', { planId: p.id })}
            >
              升级到 {p.name}
            </button>
          </div>
