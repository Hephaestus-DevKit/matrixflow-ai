...
export default function WorkflowListPage() {
  const { data: wfs, isLoading } = useQuery({ queryKey: ['wfs'], queryFn: () => apiClient.get<any[]>('/workflows') });
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between border-b border-border/40 pb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">通过拖拽画布可视化编排和运行您的自动化 AI 流水线</p>
        </div>
      ...
      {!isLoading && wfs && wfs.length === 0 && (
        <div className="rounded-xl border border-dashed border-border/60 bg-muted/10 p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground max-w-[280px]">使用流程画布将 AI 角色、逻辑判定和第三方 API 粘合为一条自动化生产线。</p>
          <Link href="/dashboard/workflows/new" className="mt-4">
            <Button size="sm" variant="outline" className="text-xs">创建第一个工作流</Button>
          </Link>
        </div>
      )}
          ...
          {wfs.map((w) => (
            ...
            >
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <h3 className="font-bold text-foreground text-sm group-hover:text-primary transition-colors">{w.name}</h3>
