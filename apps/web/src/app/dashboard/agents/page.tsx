    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between border-b border-border/40 pb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">创建、分配并协同您团队中具备专业领域认知的 AI 员工</p>
        </div>
      ...
      {!isLoading && (!agents || agents.length === 0) && (
        <div className="rounded-xl border border-dashed border-border/60 bg-muted/10 p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground max-w-[280px]">您目前没有部署任何 AI 员工。点击下方从官方岗位模板库一键部署。</p>
          <Link href="/dashboard/agents/new" className="mt-4">
            <Button size="sm" variant="outline" className="text-xs">从模板部署</Button>
          </Link>
        </div>
      )}
          ...
          {agents.map((a: any) => (
            ...
            >
              <div>
                ...
                <h3 className="mt-4 font-bold text-foreground text-sm group-hover:text-primary transition-colors">{a.name}</h3>
                <p className="text-2xs text-muted-foreground mt-0.5 uppercase tracking-wide">{a.role}</p>
              </div>
