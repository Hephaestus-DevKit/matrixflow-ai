  ...
  if (!agent) {
    return (
      <div className="rounded-xl border border-dashed border-border/60 bg-muted/10 p-12 text-center flex flex-col items-center justify-center animate-fade-in">
        ...
        <p className="mt-1 text-xs text-muted-foreground max-w-[280px]">该员工可能已被删除，或您没有访问权限。</p>
        <Button variant="outline" size="sm" onClick={() => router.push('/dashboard/agents')} className="mt-4 text-xs">
          返回员工列表
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-fade-in">
      ...
      <div className="flex items-center gap-3 border-b border-border/40 pb-5">
        <button onClick={() => router.push('/dashboard/agents')} className="text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 uppercase tracking-wide font-medium">{agent.role}</p>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-xl border border-border/60 bg-card p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs font-mono font-bold mt-0.5 text-foreground">{agent.model ?? 'glm-4-flash'}</p>
              </div>
              <div>
                <p className="text-[10px] font-semibold text-muted-foreground">所属组织 ID</p>
                <p className="text-xs font-mono mt-0.5 text-muted-foreground truncate">{agent.organizationId}</p>
              </div>
              <div>
                <p className="text-[10px] font-semibold text-muted-foreground">部署时间</p>
                <p className="text-xs mt-0.5 text-foreground">{new Date(agent.createdAt).toLocaleString('zh-CN', { hour12: false })}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border/60 bg-card p-5 shadow-sm space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {agent.skills?.map((s: any) => (
                <span key={s.skillKey} className="rounded-md bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground border border-border/50">
                  {s.skillKey}
                </span>
              ))}
            ...
            {agent.runs && agent.runs.length > 0 && (
              <div className="space-y-2">
                {agent.runs.slice(0, 10).map((r: any) => (
                  <div key={r.id} className="flex items-center justify-between rounded-xl border border-border/60 bg-card p-4 transition-all hover:border-border/80">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-mono text-2xs text-muted-foreground truncate max-w-[120px]">{r.id}</p>
