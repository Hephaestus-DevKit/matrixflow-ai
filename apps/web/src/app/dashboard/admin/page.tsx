          ...
          <>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <p className="mt-2 text-2xl font-bold text-destructive">
                ${revenue.aiCostUsd?.toFixed(2) ?? '0.00'}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-sm text-muted-foreground">市场收入</p>
              <p className="mt-2 text-2xl font-bold text-success">
                ${revenue.marketplaceRevenueUsd?.toFixed(2) ?? '0.00'}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-sm text-muted-foreground">Token 输入/输出</p>
              <p className="mt-2 text-2xl font-bold">
                {((revenue.tokenIn ?? 0) / 1000).toFixed(0)}K /{' '}
                {((revenue.tokenOut ?? 0) / 1000).toFixed(0)}K
              </p>
            </div>
          </>
        )}
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/50">
              <tr>
                ...
                <th className="px-4 py-2 text-right">Token 输入</th>
                <th className="px-4 py-2 text-right">Token 输出</th>
              ...
              {models?.map((m: any) => (
                <tr key={m.provider + m.model} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-2 text-right">
                    {m._sum?.inputTokens?.toLocaleString() ?? 0}
                  </td>
                  <td className="px-4 py-2 text-right">
                    {m._sum?.outputTokens?.toLocaleString() ?? 0}
                  </td>
            ...
            >
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => apiClient.post(`/admin/items/${item.id}/approve`, {})}
                >
                  批准
                </Button>
                <Button
                  ...
                  onClick={() =>
                    apiClient.post(`/admin/items/${item.id}/reject`, { reason: '不符合标准' })
                  }
                >
                  拒绝
                </Button>
