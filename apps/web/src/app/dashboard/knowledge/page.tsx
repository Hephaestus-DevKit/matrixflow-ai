...
export default function KbListPage() {
  const { data: kbs, isLoading } = useQuery({ queryKey: ['kb'], queryFn: () => apiClient.get<any[]>('/kb') });
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between border-b border-border/40 pb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">上传并管理您的产品说明书、问答及历史宣发资料</p>
        </div>
      ...
      {!isLoading && kbs && kbs.length === 0 && (
        <div className="rounded-xl border border-dashed border-border/60 bg-muted/10 p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground max-w-[280px]">新建一个专属知识库并上传企业文档，赋予 AI 员工深度业务认知。</p>
          <Link href="/dashboard/knowledge/new" className="mt-4">
            <Button size="sm" variant="outline" className="text-xs">创建第一个知识库</Button>
          </Link>
        </div>
      )}
          ...
          {kbs.map((kb) => (
            ...
            >
              <div>
                ...
                <h3 className="font-bold text-foreground text-sm group-hover:text-primary transition-colors">{kb.name}</h3>
